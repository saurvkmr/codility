...
fun main() {
    println(alpha(intArrayOf(2, 2, 1, 0, 1)))
}

private fun alpha(A: IntArray): Int {
    var uniqueElems = A.toHashSet()
    for (i in A.indices) {
        if (uniqueElems.isNotEmpty()) {
            if (uniqueElems.contains(A[i]))
                uniqueElems.remove(A[i])
        } else
            return i - 1
    }
    return A.size - 1
